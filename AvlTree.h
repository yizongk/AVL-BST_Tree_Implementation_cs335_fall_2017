...
class AvlTree
{
  public:
    ...
    // Added by Yi Zong Kuang, to aid in project.
    // Remove function that keeps track of # of recursion being called.
    void remove( const Comparable & x, int & recur_tracker, bool & found_flag )
    {
        remove( x, root, recur_tracker, found_flag );
    }

    // Find and return element from tree.
    const Comparable* find( const Comparable & x ) const {
      return find( x , root );
    }
 
    // Find and return element from tree, while keeping track of # of recursion calls.
    ...
    // Find the depth of each node, and calculate the average of it.
    float calAvgDepth() {
  ...
  private:
    ...
    void insert( const Comparable & x, AvlNode * & t )
    {
        if( t == nullptr )
            ...
            t->element.Merge(x);// Not suppose to do anything.But for the purpose of this program, we gonna merg the datas if recognition ID are the same.
...
*****************************************************/
    ...
    // Remove an node from thet tree, while keeping track of # of recursion and if the node to be removed exists.
    void remove( const Comparable & x, AvlNode * & t, int & recur_tracker, bool & found_flag )
    {
        ++recur_tracker;
        if( t == nullptr ) {
            found_flag = false;
    ...
    // Find and return an element from the tree and keeps track of # of recursions being called.
    const Comparable* find( const Comparable & x, AvlNode *t , int & recur_tracker ) const {
     ++recur_tracker;
      if( t == nullptr )
          return nullptr;
      else if( x < t->element )
    ...
    // Find the number of nodes in the trees.
    unsigned int numOfNodes( AvlNode *t ) {
      if( t != nullptr ) 
    ...
    // Helper to calculate average depth of the every node in the tree.
    void calAvgDepthHelper(AvlNode *t, int & depth_calculate, float & allDepths, float & num_of_nodes) {
      if( t != nullptr )
        {
            ++num_of_nodes;
            ++depth_calculate;
